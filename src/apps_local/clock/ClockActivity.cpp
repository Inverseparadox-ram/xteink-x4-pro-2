...
void ClockActivity::refreshModel() {
  ...
  uint8_t hour = 0;
  uint8_t minute = 0;
  // A year before 2020 means the chip was never set -- HalClock::begin()
  // applies the same test before trusting it for system time.
  clockValid_ = halClock.getDateTime(year, month, day, hour, minute) && year >= 2020;
  if (clockValid_) {
    clockapp::Civil utc;
    utc.year = year;
    utc.month = month;
    ...
    utc.hour = hour;
    utc.minute = minute;
    // The RTC holds UTC (HalClock::begin says so, and the NTP path writes it
    // that way), so the whole timestamp is shifted -- not just the hours. A
    // reader that shifted only the clock face would sit above a calendar
    // highlighting the wrong day for hours at a time.
    local_ = clockapp::toLocal(utc, SETTINGS.clockUtcOffsetQ);
