...
def main(env):
    ...
    # CrossPlay 1.13.9 moved the device HalClock onto localTime(), which hands
    # back a struct tm under the POSIX rule setTimezone() installed -- the Clock
    # app reads the DATE from it, not just the clock face. The simulator's shim
    # has getTime/getDateTime and nothing that returns a tm, so it needs the
    # same method or apps_local/clock does not sim-compile.
    #
    # localtime_r rather than gmtime_r, and that is the whole point: the patch
    # above already puts the chosen rule in the process TZ, so the simulator
    # resolves it through the same libc path the device does.
    patch(
        src / "HalClock.h",
        "  bool syncFromNTP();",
